...
usr_prompt = """You are creating a meta title and meta description for an ecommerce category page.
...
def update_db():
    ...
    print("Aktualizacja default_prompt_steps...")
    res1 = client.table("default_prompt_steps").update({
        "system_prompt": sys_prompt,
        "user_prompt": usr_prompt
    }).eq("step_key", "meta_titles_and_descriptions").execute()
    print(f"Zaktualizowano {len(res1.data)} rekordów w default_prompt_steps.")

    print("Aktualizacja campaign_prompt_steps...")
    res2 = client.table("campaign_prompt_steps").update({
        "system_prompt": sys_prompt,
        "user_prompt": usr_prompt
    }).eq("step_key", "meta_titles_and_descriptions").execute()
    print(f"Zaktualizowano {len(res2.data)} rekordów w campaign_prompt_steps.")
